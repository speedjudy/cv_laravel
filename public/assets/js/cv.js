    ...
    $("#searchCV").click(function () {
        $.get(
            ...
            function (res) {
                ...
                var include_unavailable = $("#include_unavailable")[0].checked+"";
                var html = "";
                ...
                for (var i = 0; i < cvDatas.length; i++) {
                    ...
                    if (include_unavailable!=cvDatas[i]['cv_indispo'].toLowerCase()) {
                        flag = false;
                    }
                    if (cvDatas[i]["cv_dispo"]) {
                    ...
                    console.log(profils_data, profils, cvDatas);
                    if (profils_data.length > 0 && profils.length > 0) {
                        for (var k = 0; k < profils_data.length; k++) {
                            var c_flag = false;
                            for (var j = 0; j < profils.length; j++) {
                                console.log(profils_data[k]["id_cv"], cvDatas[i]["id_cv"], profils_data[k]["id_profil"], profils[j]);
            ...
            function (res) {
                ...
                for (var i = 0; i < res.length; i++) {
                    ...
                    html +=
                        "<td><a href='uploads/" +
                        res[i]["cv_file"] +
                        "' target='_blank' id='downloadCV'><i class='fas fa-download'></i></a> &nbsp;&nbsp;<a href='javascript:void(0);' id='editCV'><i class='fas fa-edit'></i></a> &nbsp;&nbsp; <a href='javascript:void(0);' id=removeCV><i class='fas fa-trash'></i></a></td>";
                    html += "</tr>";
                    if (n_id < res[i]["id_cv"]) {
                        n_id = res[i]["id_cv"];
                    }
